...
function Product({id, name, image, newprice, oldprice, offer, rating, enterProductDetail}) {
    const dispatch = useDispatch();
    const history = useHistory();

    const detailproduct = () => {
        if(id){
            dispatch(enterProductDetail({
                productDetailId: id
            }))
        }
        history.push(`/healthcare/product/${id}`);
    }
    return (
        <div onClick={detailproduct} className=" 
